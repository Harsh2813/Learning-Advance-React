...
const Login = (props) => {
  ...
  const autCtx = useContext(AuthContext);// used useContext
  ...
  useEffect(() => {
    // first time ye useEffect run hoga & check karega form valid ka agr mail me @ aur password >6 h0
    const identifier = setTimeout(() => {
      // is settimeour ki vajah se turant serFormValid run ni ho rha isiliye return me clearTimeout kr rhe taki ye setTime na ho abhi jaise hi user stop karega typing 3 sec baad valid chek hoga
    ...
    return () => {
      // this we used cleanUp fn of useEffect we have to use return like this inside first args of useEffect and whatever we are adding above we can remove here in return and return takes a fn.
      console.log("cleanUp");
  ...
  const submitHandler = (event) => {
    ...
    if(formIsValid){
      autCtx.onLogin(emailState.value, /*enteredPassword*/ passwordState.value); //NOTE for useReducer we changed enteredEmail to emailState.value and in place of this (enteredEmail.includes('@') and emailIsValid, we used emailState.isValid because we changed from useState to useReducer
    }
    else if(!formIsValid){
    ...
    <Card className={classes.login}>
      <form onSubmit={submitHandler}>
        <Input
          ref={emailInputRef}
          id="Email"
          label="E-mail"//we create separated input compo
          type="email"
